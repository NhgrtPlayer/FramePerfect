...
namespace WebCrawlerManager
{
    ...
    class DustLoopCrawler : IWebCrawler
    {
        ...
        public void Crawl()
        {
            for (int gameIdx = 0; gameIdx < games.Count; gameIdx++)
            {
                ...
                foreach (string characterUrl in charactersUrls)
                {
                    ...
                    CrawlOnTimer.logs.Info("CharacterName:" + character.Name);
                    ...
                    {
                        CrawlOnTimer.logs.Info("GameIDx:" + games[gameIdx] + ":" + character.Name + ":" + tableNb + ":" + nodeTables.Count() + ":" + characters.Count);
                        var moveTypeNode = table.PreviousSibling;
                        //CrawlOnTimer.logs.Info("MoveTypeNode:" + moveTypeNode.InnerText);
                        while (moveTypeNode != null && moveTypeNode.Name.IndexOf("h2") < 0)
                        {
                            moveTypeNode = moveTypeNode.PreviousSibling;
                            //CrawlOnTimer.logs.Info("MT:" + moveTypeNode.Name);
                        }
                        string moveType = "";
                        if (moveTypeNode != null)
                        {
                            ...
                            CrawlOnTimer.logs.Info("MoveType:" + moveType + ":" + ":" + moveTypeNode.Name + ":" + moveTypeNode.XPath);
                        }
                        //                        moveType = moveType.Substring(0, moveType.IndexOf("[edit]"));
                        int loopNb = 0;
                        List<MoveValue> tabMoveCorrespondance = new List<MoveValue>();
                        //CrawlOnTimer.logs.Info("TrNb:" + table.Descendants("tr").Count());
                        foreach (var tr in table.Descendants("tr"))
                        {
                            if (loopNb == 0)
                            {
                                ...
                                {
                                    //CrawlOnTimer.logs.Info(th.Name + ":" + th.XPath);
                    ...
                    CrawlOnTimer.logs.Info("Character " + character.Name + " crawled");
                }
        ...
        public void AssignMoveValue(Move move, string value, MoveValue mValue)
        {
            value = value.Replace("\n", "");
            value = value.Replace("'", " ");
            //CrawlOnTimer.logs.Info(value + ":" + mValue);
            switch (mValue)
            {
